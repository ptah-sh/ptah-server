...
const props = defineProps({
    node: Object,
    swarms: Array,
    swarmsQuotaReached: Boolean,
});

const swarmOption = reactive({
    form: props.swarmsQuotaReached ? "join" : "init",
});
...
<template>
    <FormSection @submitted="submit">
        ...
        <template #description>
            Select either to initialize a new cluster or join an existing
            cluster.
        </template>
        ...
        <template #form>
                ...
                >
                    ...
                    >
                        <div class="flex items-center ps-3">
                            <input
                                id="swarm-init-option"
                                v-model="swarmOption.form"
                                type="radio"
                                value="init"
                                name="list-radio"
                    ...
                    >
                        <div class="flex items-center ps-3">
                            ...
                            />
                            <label
                                for="swarm-join-option"
                                class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                                >Join an Existing Cluster</label
                            >
                        </div>
                    </li>
                </ul>
            ...
            <div v-auto-animate class="col-span-6 sm:col-span-4">
                <div v-if="swarmOption.form === 'init'" class="grid gap-4">
                    <template v-if="swarmsQuotaReached">
                        <p class="text-sm text-red-500">
                            You have reached the maximum number of Swarm
                            Clusters allowed.
                        </p>
                    </template>
                    <template
                        v-else-if="$props.node.data.host.networks.length > 0"
                    >
                ...
                <div
                    v-if="swarmOption.form === 'join'"
                    v-auto-animate
                    class="grid gap-4"
                >
                    <template v-if="swarms.length === 0">
                        <p class="text-sm text-red-500">
                            Please initialize a cluster first - no Swarm
                            Clusters exist yet.
                        </p>
                    </template>
                    <template>
                        <FormField :error="joinForm.errors.swarm_id">
                            <template #label> Swarm Cluster </template>

                            <Select v-model="joinForm.swarm_id">
                                <option
                                    v-for="swarm in $page.props.swarms"
                                    :value="swarm.id"
                                >
                                    {{ swarm.name }}
                                </option>
