...
const submitForm = async () => {
    ...
    for (const registry of formData.registries) {
        ...
        encryptedFormData.registries.push(registryData);
    }

    form.transform(() => encryptedFormData).post(
        route("swarms.update-docker-registries", {
...
<template>
    <FormSection @submit="submitForm" :task-group="taskGroup">
        <template #title> Docker Registries </template>

        <template #description>
            Configure your custom Docker Registries to be able to pull images
            not only from the Docker Hub.
        </template>

        <template #form>
            <div v-if="form.registries.length === 0" class="col-span-full">
                There are no custom registries yet.
            </div>
            <template
                v-for="(registry, index) in form.registries"
                :key="registry.id"
                class=""
            >
                ...
                <FormField
                    :error="form.errors['registries.' + index + '.name']"
                    class="col-span-2"
                >
                ...
                <FormField
                    :error="
                        form.errors['registries.' + index + '.serverAddress']
                    "
                    class="col-span-4"
                >
                    <template #label> Server Address </template>

                    <div class="flex gap-2">
                        <TextInput
                            v-model="registry.serverAddress"
                            class="w-full"
                            placeholder="registry.gitlab.com"
                        />
                        <SecondaryButton
                            @click="form.registries.splice(index, 1)"
                            tabindex="-1"
                        >
                            <svg
                ...
                <FormField
                    :error="form.errors['registries.' + index + '.username']"
                    class="col-span-2"
                >
                ...
                <FormField
                    :error="form.errors['registries.' + index + '.password']"
                    class="col-span-2"
                >
